...
- CI: added `scripts/ci_validate_templates.py` and a `validate-templates` GitHub Actions job which runs on push/PR and validates template syntax, undefined variables, and manifests (including HMAC checks). Build-on-tag `build-artifacts` job already exists to produce sdist/wheel and upload them as artifacts.
