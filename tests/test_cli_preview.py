...
def test_list_templates_details(tmp_path, monkeypatch):
    ...
    runner = CliRunner()
    result = runner.invoke(cli, ['list-templates', '--templates-dir', str(tpl_root), '--details'])
...
def test_preview_template_raw_and_render(tmp_path, monkeypatch):
    ...
    runner = CliRunner()
    # raw
    result = runner.invoke(cli, ['preview-template', 'bar', '--file', 'hello.txt.j2', '--templates-dir', str(tpl_root)])
    assert result.exit_code == 0
